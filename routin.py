...
from .models import Products, RoutinePlan, RoutineStep
...
def create_product(db: Session, payload: ProductCreate) -> Products:
    product = Products(
        name=payload.name,
        brand=payload.brand,
...
def list_products(
    ...
    if price_range:
        lo, hi = price_range
        query = query.filter(Products.price >= lo, Products.price <= hi)
    return query.all()

def score_product(p: Products, skin_type: str, concerns: List[str], preferences: List[str]) -> float:
    score = 0
...
def select_best_product(db: Session, category: str, skin_type: str, concerns: List[str], preferences: List[str], budget: List[float]) -> Optional[Products]:
    # lo, hi = budget
